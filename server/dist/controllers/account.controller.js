...
export const transferMoney = async (req, res, next) => {
    try {
        const { transferType, sourceAccountId, amount, pin, 
        // Local transfer specific
        beneficiaryName, beneficiaryAccountNumber, bankName, transferMethod, 
        ...
        if (transferType === 'local') {
            ...
            result = await executeTransfer(req.user.id, sourceAccountId, amount, sourceAccount.currency, recipient.id, formattedReference);
        }
            ...
            result = await executeTransfer(req.user.id, sourceAccountId, amount, sourceAccount.currency, undefined, formattedReference);
        }
        else {
            // Fallback for legacy prototype transfers
            const { targetCurrency, recipientId } = req.body;
            result = await executeTransfer(req.user.id, sourceAccountId, amount, targetCurrency || sourceAccount.currency, recipientId, reference);
        }
        res.status(200).json({
            status: 'success',
            data: result
